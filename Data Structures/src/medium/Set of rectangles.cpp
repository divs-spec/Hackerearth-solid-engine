...
const int MAX_N = 2e5 + 121, N = 2e5 + 10;
int n, q;
int a[MAX_N],b[MAX_N],c[MAX_N],d[MAX_N],w[MAX_N],result[MAX_N];
int t[10 * MAX_N];
...
int main(){
    ...
    for(int i = 0; i < (int)events.size(); ++i){
        ...
        if(p.second.first == 0){
            ...
            int value = get(1, 0, N, pos, n - 1);
            upd(1, 0, N, pos, value + 1);
        }else{
            int pos = lower_bound(w, w + n, c[p.second.second]) - w;
            result[p.second.second] = get(1, 0, N, pos, n - 1);
        }
    }
