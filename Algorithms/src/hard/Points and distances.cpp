...
pnode reassign(pnode X, pnode L, pnode R, unsigned int p, ll v) {
	...
	if(getLeftMost(R) != INF) X -> best = min(X -> best, getLeftMost(R) - X -> rightMost);
	if(getRightMost(L) != INF) X -> best = min(X -> best, X -> leftMost - getRightMost(L));
...
int main() {
	...
	pnode root = nullptr;
	for(int i = 1; i <= n; i++) {
		if(root == nullptr) root = reassign(new node(), nullptr, nullptr, generate(), ar[i]);
		else root = insert(root, reassign(new node(), nullptr, nullptr, generate(), ar[i]));
	}
	while(q--) {
		int type;
		scanf("%d", &type);
